...
class DataRegionalRepository implements RegionalRepository {
    ...
    public function getRegional($id_regional): array {
        $sql = "SELECT reg.*
                FROM regional reg
                WHERE reg.id=:id_regional AND reg.activo=1";
        ...
        if($res->rowCount()>0){
            ...
            $res = $res[0];
            $data_dep = $this->dataParametricaRepository->getCodParametrica('param_departamentos_bol',0,$res['departamento']);
            $result = array('id'=>$res['id'],
                            'codigo'=>$res['codigo'],
                            'nombre'=>$res['nombre'],
                            'departamento'=>$data_dep,
    ...
    public function listRegional($query): array {
        ...
        if($res->rowCount()>0){
            ...
            $arrayres = array();
            foreach ($res as $item){
                $data_dep = $this->dataParametricaRepository->getCodParametrica('param_departamentos_bol',0,$item['departamento']);
